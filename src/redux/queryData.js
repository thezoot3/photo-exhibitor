    ...
    switch(action.type) {
        case MODIFY_QUERY:
            if(state.registered.has(action.query)) {
                let expType;
                if(expType = state.query[action.query].expectType) { //할당 후 평가되는 평가식임 문제 없음!
        ...
        case REGISTER_QUERY:
            ...
            if(!state.registered.has(action.query)) {
                ...
                let set = state.registered.values()
                return {
                    ...state,
        ...
        case UNREGISTER_QUERY:
            if(state.registered.has(action.query)) {
                return {
                    ...state,
                    registered: state.registered.delete(action.query),
                    query: {
